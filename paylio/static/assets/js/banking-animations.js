...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        initStarfieldAnimation(); // NEW: Galaxy stars animation
    ...
    function initStarfieldAnimation() {
        // Apply starfield only to testimonials section
        const sections = $('.testimonials');
        console.log('🌟 Starfield: Found testimonials section:', sections.length);

        sections.each(function () {
            const section = $(this);
            const canvas = $('<canvas class="starfield-canvas"></canvas>');
            ...
            console.log('🌟 Starfield: Canvas dimensions:', width, 'x', height);

            const stars = [];
            const starCount = 150;

            // Create stars with varying sizes and speeds for depth
            for (let i = 0; i < starCount; i++) {
                ...
                stars.push({
                    ...
                    y: Math.random() * canvas[0].height,
                    radius: size,
                    vx: (Math.random() - 0.5) * (size * 0.5),
                    vy: (Math.random() - 0.5) * (size * 0.5),
            ...
            function drawStarfield() {
                ...
                stars.forEach((star, index) => {
                    ...
                    // Wrap around edges
                    if (star.x < 0) star.x = canvas[0].width;
                    if (star.x > canvas[0].width) star.x = 0;
                    if (star.y < 0) star.y = canvas[0].height;
                    if (star.y > canvas[0].height) star.y = 0;

                    // Twinkling effect
                    const twinkle = Math.sin(twinklePhase + index * star.twinkleSpeed) * 0.3 + 0.7;
                    ...
                    gradient.addColorStop(0.3, 'rgba(0, 119, 182, 0.4)');
            ...
            // Resize handler
            $(window).on('resize', () => {
                canvas[0].width = section.outerWidth();
                canvas[0].height = section.outerHeight();
            });
        });
    }
